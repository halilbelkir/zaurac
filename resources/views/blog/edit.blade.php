                ...
                <form action="{{route('blog.update',$blog->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2">
                        <div>
                            ...
                            @else
                                <x-jet-input id="title" name="title" onkeyup="seflink1(this.value,'#seflink')" type="text" value="{{$blog->title}}" class="mt-1 block"  placeholder="Başlık" />
                            @endif
                        ...
                        <div>
                            ...
                            @else
                                <x-jet-input id="seflink" name="seflink" type="text" value="{{$blog->seflink}}" class="mt-1 block"  placeholder="Seflink" />
                            @endif
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" class="mt-1 mb-1 block border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"  placeholder="İçerik" rows="2">@if(empty(old('content'))) {{$blog->content}} @else {{old('content')}} @endif</textarea>
                        <x-jet-input-error for="content" class="mt-2" />
                    </div>

                    <div>
                        <x-jet-label for="youtube_link" value="Youtube Linkleri (Her linki virgül ile ayırınız.)" />
                        <textarea id="youtube_link" name="youtube_link" maxlength="160" class="mt-1 mb-1 block border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"  placeholder="Youtube Linkleri (Her linki virgül ile ayırınız.)" rows="2">@if(empty(old('youtube_link'))) {{$blog->youtube_link}} @else {{old('youtube_link')}} @endif</textarea>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 mb-3">
                        <div>
                            ...
                            @else
                                <x-jet-input id="tag" name="tag" type="text" value="{{$blog->tag}}" class="mt-1 block"  placeholder="Etiket (Kelimeleri virgül ile ayırınız.)" />
                            @endif
